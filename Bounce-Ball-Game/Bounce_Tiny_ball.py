...
root = Tk()
root.title("Bounce Tiny Ball Game")
root.geometry("500x570")
root.resizable(0, 0)
root.wm_attributes("-topmost", 1)
canvas = Canvas(root, width=500, height=500, bd=0, highlightthickness=0, highlightbackground="Red", bg="Green")
canvas.pack(padx=10, pady=10)
score = Label(height=50, width=80, text="Score: 00", font="Calibri 25 italic")
        ...
        while 1:
            if pole.pauseSeconds != 1:
                ...
                if not ball.bottom_hit:
                    ...
                    time.sleep(0.01)
                    if ball.hit == 95:
                        canvas.create_text(250, 250, text="YOU WON !!", fill="yellow", font="Calibri 50 ")
                        root.update_idletasks()
                        root.update()
                        playing = False
                        break
                else:
                    canvas.create_text(250, 250, text="GAME OVER!!", fill="red", font="Calibri 50 ")
                    root.update_idletasks()
                    root.update()
                    playing = False
                    break
                ...
                try:
                    ...
                    m = canvas.create_text(250, 250, text="PAUSE!!", fill="green", font="Calibri 50 ")
                root.update_idletasks()
                root.update()


root.bind_all("<Return>", start_game)
canvas.create_text(250, 250, text="Press Enter to start Game!!", fill="yellow", font="Calibri 25")
j = canvas.find_all()
root.mainloop()
